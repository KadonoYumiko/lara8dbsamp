...
<div class="flex justify-end mb-10">
    <a href="kadai06_1/create" class="block w-24 text-white text-center bg-sky-600 hover:bg-sky-500 px-3 py-2 rounded-md">新規投稿</a>
</div>
...
<section class="grid grid-cols-1 lg:grid-cols-3 gap-10 mb-10">
    ...
    <article class=" bg-white hover:bg-white rounded-md shadow-md hover:shadow-lg transition-shadow">
        <a href="kadai06_1/{{ $article->id }}" class="block w-full h-full">
            {{-- 画像は後々表示実装 --}}
            {{-- <figure>
                <img src="">
            </figure> --}}
            ...
            <p class="text-gray-400 text-xs mb-5 px-2"><time datetime="{{ $article->created_at }}">{{ $article->created_at }}</time></p>
        </a>
    </article>
    @endforeach
</section>
